// Smoke test: KeetaRepository address-book persistence (shared_preferences).
//
...
void main() {
  ...
  test('upserted address survives a fresh KeetaRepository, delete removes it',
      () async {
    // First repo: load the seed book, then add a new address (persisted).
    final repo1 = KeetaRepository();
    await repo1.load();
  ...
  test('defaultAddress/activeAddress never throw on an empty address book', () {
    ...
    expect(repo.defaultAddress, same(KeetaAddress.empty));
    expect(() => repo.activeAddress, returnsNormally);
    expect(repo.activeAddress, same(KeetaAddress.empty));
  });

  test('deleteAddress keeps the last remaining address (book never emptied)',
      () async {
    final repo = KeetaRepository();
    await repo.load();
