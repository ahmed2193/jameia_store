...
// `ProductDetailPage` for a real product and verifies:
...
//
// Network thumbnails (JameiaImage) resolve to placeholders under test — expected
// and must not throw.
...
import 'package:jameia_mart/src/features/product_details/product_details_injection_container.dart';
...
void main() {
  ...
  testWidgets('sticky Add-to-cart adds the product to the cart', (
    tester,
  ) async {
    final product = repo.allProducts.firstWhere(
      (p) => !p.hasVariants && p.available,
