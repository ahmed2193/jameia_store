...
void main() {
  test(
    'parseJameiaCatalog builds categories/sections/products with VIP price',
    () {
      final raw = File(
      ...
      final withRanks = cat.categories
          .expand((c) => c.subs)
          .where(
            (s) => s.hasRanks && s.ranks.any((r) => r.products.isNotEmpty),
          );
      ...
      final vipCount = cat.productsBySku.values
          .where((p) => p.hasVipPrice)
          .length;
      expect(
        vipCount,
        greaterThan(500),
        reason: 'differing VIP prices should be merged',
      );
