// Smoke tests for the rebuilt shop menu (jm3eia data-flow + Keeta UI).
//
// Boots the real service locator (loads the bundled jm3eia catalog), then mounts
// `ShopScreen` and verifies the new scroll architecture:
...
//
// Network thumbnails (KeetaImage) resolve to placeholders under test — that is
...
import 'package:jameia_mart/core/data/jameia/jameia_models.dart';
import 'package:jameia_mart/core/data/catalog_constants.dart';
...
import 'package:jameia_mart/features/shop/presentation/screens/shop_screen.dart';
import 'package:jameia_mart/features/shop/shop_injection_container.dart';
...
import 'package:jameia_mart/features/shop/presentation/widgets/rank_rail.dart';
import 'package:jameia_mart/features/shop/presentation/widgets/shop_hero.dart';
...
void main() {
  ...
  Future<void> pumpShop(WidgetTester tester, String shopId, CartCubit cubit) async {
  ...
  testWidgets('catalog produced parent shops with an IMAGE rank rail', (tester) async {
    ...
    final withImageRail = repo.shops
        .where((s) => s.sections.length > 1 &&
            s.sections.any((sec) => sec.image.isNotEmpty))
        .toList();
    expect(withImageRail, isNotEmpty,
        reason: 'expected parent categories rebuilt as image-rail rank shops');
  });

  testWidgets('ShopScreen builds a ranked shop with no exception + renders content',
      (tester) async {
    ...
    final firstSection =
        shop.sections.firstWhere((sec) => sec.products.isNotEmpty);
  ...
  testWidgets('cart bar reacts when a no-variant product is added', (tester) async {
    final shop = repo.shops.firstWhere(
      (s) => s.sections.any(
          (sec) => sec.products.any((p) => !p.hasVariants)),
      orElse: () => repo.shops.first,
    );
  ...
  testWidgets('tapping a rank rail item scrolls the body to that section',
      (tester) async {
    final shop = repo.shops.firstWhere(
      (s) => s.sections.length >= 3 &&
          s.sections.every((sec) => sec.products.isNotEmpty),
      orElse: () => repo.shops.firstWhere((s) => s.sections.length > 1,
          orElse: () => repo.shops.first),
    );
    final cubit = CartCubit();
    addTearDown(cubit.close);
  ...
  testWidgets('multi-sub category swaps its body when another sub-tab is tapped',
    ...
    // ShopScreen takes the CATEGORY id; its subs become the pinned sub-tabs.
    ...
    // COLLAPSED hero — see ShopScreen._swapSub / _subTabDockOffset. It does NOT
    ...
    expect(tester.takeException(), isNull);
    expect(find.byType(HeroBg), findsNothing); // hero stays collapsed at the dock
    expect(controller.offset, isNot(beforeSwap)); // swap repositioned the scroll
