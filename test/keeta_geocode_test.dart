// Smoke test: KeetaGeocode offline LBS contract.
//
...
void main() {
  ...
  test('reverse(base) returns non-empty area + line + plusCode', () {
    final r = KeetaGeocode.reverse(base);
  ...
  test('isServiceable(base) == true; isServiceable(0,0) == false', () {
    expect(KeetaGeocode.isServiceable(base), isTrue);
    expect(KeetaGeocode.isServiceable(const LatLng(0, 0)), isFalse);
  });

  test('nearbyCandidates(base) returns 3 entries, each with its own pos', () {
    final c = KeetaGeocode.nearbyCandidates(base);
    expect(c.length, 3);
    ...
    final positions = c.map((e) => '${e.pos.latitude},${e.pos.longitude}').toSet();
