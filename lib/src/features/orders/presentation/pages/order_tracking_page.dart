...
/// Jameia order tracking / order-detail screen (`mach_pro_sailor_c_order_status_global`).
///
/// 1:1 clone of the live-tracking page: styled live-map placeholder (real Jameia
/// rider/shop/user markers), confirm→prepare→pickup→on-the-way→delivered progress
/// stepper bound to [JameiaOrder.statusStep], ETA bubble, rider action bar
/// (call + chat), order summary (items / total / payment), delivery address from
/// [JameiaRepository.defaultAddress], and a help / refund entry.
///
/// [JameiaOrder] has no payment/address fields, so the payment row uses a static
/// label and the address row reads the default address.
class OrderTrackingPage extends StatelessWidget {
  const OrderTrackingPage({super.key, this.orderId = 'o1'});
