...
/// Jameia refund progress / detail screen (`mach_pro_sailor_c_order_refund_detail`).
///
...
/// Jameia's `refund_progress_part_waiting/processing/success/fail` states), a
...
/// goes back), and a help / FAQ entry (Jameia's `jumpToSuccessRate` + customer
/// service jump).
///
/// [JameiaOrder] carries no refund fields, so the refund amount / method are
/// derived from the order total with static demo deductions.
class OrderRefundDetailPage extends StatelessWidget {
  const OrderRefundDetailPage({super.key, this.orderId = 'o1'});
      ...
      child: Row(
        children: [
          Container(
            ...
            child: Icon(
              done ? JameiaIcons.confirm : JameiaIcons.refund,
              size: 24,
...
/// Jameia shows a "refunded to your wallet by default" toast/note on this screen.
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Row(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Icon(JameiaIcons.info, size: 14, color: AppColors.tertiaryText),
  ...
  Widget build(BuildContext context) {
    return _Card(
      child: Column(
        children: [
          _HelpRow(
            icon: JameiaIcons.help,
            label: 'orders.refund_faq'.tr(),
            onTap: () => context.push(Routes.customerServiceQuestion),
          ),
          const ThinDivider(),
          _HelpRow(
