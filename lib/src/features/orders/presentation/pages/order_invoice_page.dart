...
/// Jameia invoice / e-receipt screen (`mach_pro_sailor_c_order_invoice`).
///
...
/// [JameiaRepository.orders] `.first`), an invoice-title chooser row (the live app
...
///
/// [JameiaOrder] carries no tax/seller fields, so the e-invoice meta rows use the
/// fixed demo values Jameia shows for a personal e-receipt.
class OrderInvoicePage extends StatelessWidget {
  const OrderInvoicePage({super.key, this.orderId});
...
class _ReceiptHeader extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Row(
      children: <Widget>[
        JameiaImage.circle(url: order.shopLogo, size: 40),
...
class _ReceiptTotals extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    // [JameiaOrder.total] is the charged amount; derive demo subtotal + fees so
    // the receipt reads like a real Jameia breakdown (subtotal + fee + tax = total).
...
class _InvoiceTitleRow extends StatelessWidget {
  ...
  Future<void> _openChooser(
    ...
    // Real Jameia drives this as a CENTER modal (black 60% overlay + scale-in),
    // not a bottom sheet. [showJameiaDialog] supplies the fade + scale transition;
    // the dialog container itself uses a uniform 8dp radius (bundle class g6a017).
    final picked = await showJameiaDialog<String>(
      context,
