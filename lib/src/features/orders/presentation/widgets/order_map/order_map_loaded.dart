...
class _OrderMapLoadedState extends State<OrderMapLoaded> {
  ...
  /// Real Jameia asset markers, loaded asynchronously (null until ready — we fall
  ...
  late List<LatLng> _route = JameiaGeocode.routePolyline(
    _kShopLatLng,
  ...
  Future<void> _loadMarkers() async {
    ...
    final results = await Future.wait([
      ...
      JameiaMap.headingMarker(
        ...
        _riderPos,
        isCar
            ? JameiaAssets.carMarkerForHeading(_riderHeading)
  ...
  Future<void> _refreshRiderMarker() async {
    final order = widget.order;
    final isCar = order.rider?.vehicle == 'car';
    final rider = await JameiaMap.headingMarker(
      'rider',
      _riderPos,
      isCar
          ? JameiaAssets.carMarkerForHeading(_riderHeading)
  ...
  void _startRiderSimulation() {
    _riderTimer?.cancel();
    _route = JameiaGeocode.routePolyline(_kShopLatLng, _riderPos, _kUserLatLng);
    _routeIdx = 0;
  ...
  Widget build(BuildContext context) {
    ...
    // Real Jameia asset markers once loaded; hue pins as a fallback so the map is
