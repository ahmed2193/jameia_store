...
/// Offline source for the order lifecycle. The live Jameia pages hit
/// `/api/order`, `v1/order/status`, `v1/order/refund`, `v1/ugc/reviews/submit`
/// etc.; here every read resolves from the in-memory [JameiaRepository] and the
...
abstract class OrdersLocalDataSource {
  ...
  JameiaOrder orderById(String id);

  /// The most recent order (`orders.first`).
  JameiaOrder firstOrder();
  ...
  String navigableShopId(JameiaOrder order);
...
class OrdersLocalDataSourceImpl implements OrdersLocalDataSource {
  ...
  @override
  List<JameiaOrder> orders() => catalog.orders;

  @override
  JameiaOrder orderById(String id) => catalog.orderById(id);

  @override
  JameiaOrder firstOrder() => catalog.orders.first;
  ...
  RefundDetail refundDetail(String id) {
    ...
    return RefundDetail(
      ...
      etaLabel: fullyRefunded
          ? 'orders.eta_done'.tr()
          : 'orders.eta_estimate'.tr(),
