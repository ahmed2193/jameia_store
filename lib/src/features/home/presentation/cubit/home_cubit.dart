...
class HomeCubit extends Cubit<HomeState> with SafeCubitMixin<HomeState> {
  ...
  Future<void> load() async {
    ...
    result.fold(
      (failure) => safeEmit(
        state.copyWith(status: HomeStatus.error, error: failure.message),
      ),
      (feed) {
        _feed = feed;
  ...
  Future<void> selectFilter(int index) async {
    ...
    final result = await _selectHomeFilter(
      SelectHomeFilterParams(
        index: index,
        filters: feed.filters,
        shops: feed.shops,
  ...
  /// SKU — routes the old inline `sl<JameiaRepository>().locationOfSku` read
