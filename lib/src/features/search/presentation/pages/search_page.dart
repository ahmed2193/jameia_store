...
/// Jameia global search entry (`c_search`) — a faithful rebuild of the real
...
/// the Jameia suggestion list. Committing a query navigates to the results page
/// ([Routes.searchShop]).
...
class _SearchViewState extends State<_SearchView> {
  ...
  void _submit(String term) {
    ...
    _cubit.addRecent(t);
    _focus.unfocus();
    context.push(Routes.searchShop, extra: t).then((_) => _resetField());
  }

  /// Coming back from results: clear the field so the entry screen shows the
  /// discover state again (Jameia returns to the empty search).
  void _resetField() {
    if (!mounted) return;
    _controller.clear();
...
/// tinted (light-red bg + red text), mirroring Jameia's hot-word highlight.
