...
/// Offline source for the global search feature. The live Jameia page hits
/// `/api/search/*` + a server-side recents store; here matches come from the
/// in-memory [JameiaRepository] catalogue and the recents list is persisted to
/// [LocalStorage] (shared_preferences) so it survives app restarts.
abstract class SearchLocalDataSource {
  /// Catalogue shops matching [query] (delegates to [JameiaRepository.searchShops]).
  List<Shop> searchShops(String query);

  /// Popular-brands strip for the discover landing — catalogue shops that have a
  /// logo, capped at the first 16 (mirrors Jameia's `popular brands` block).
...
class SearchLocalDataSourceImpl implements SearchLocalDataSource {
  ...
  static const String _recentsKey = 'jameia.search.recents.v1';
  ...
  List<String> suggestions(String query) {
    ...
    return [
      ...prefix,
      ...contains,
    ].take(_maxSuggestions).toList(growable: false);
  }

  @override
  List<String> hotWords() {
    // Derived from the most common shop tags in the catalog, mirroring Jameia's
