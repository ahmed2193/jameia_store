...
/// Jameia `mach_pro_sailor_c_fixed_price` — the fixed-price / flash ordering
...
///
/// The reference page is sourced from a single flash shop (Jameia's fixed-price
...
class FixedPricePage extends StatelessWidget {
  const FixedPricePage({super.key, this.shopId});
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<FixedPriceCubit, FixedPriceState>(
      builder: (context, state) {
        ...
        // wrap shared core widgets (JameiaImage, ProductCard) and the cross-
...
class _FlashHero extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return SliverAppBar(
      ...
      leading: const _CircleIcon(icon: JameiaIcons.back),
      actions: const [
        _CircleIcon(icon: JameiaIcons.share),
        _CircleIcon(icon: JameiaIcons.search),
...
class _CountdownPillState extends State<_CountdownPill> {
  // Fixed start: 2 h 45 m 10 s (matches the Jameia reference screenshot value).
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Padding(
        ...
        child: Row(
          children: [
            const Icon(JameiaIcons.rank, size: 18, color: AppColors.finalPrice),
            const SizedBox(width: AppSpacing.s6),
            Text(
          ...
          child: AnimatedContainer(
            ...
            child: Row(
              children: [
                ...
                GestureDetector(
                  onTap: active
                      ? () => context.push(Routes.checkout, extra: shop.id)
                      : null,
                  child: Container(
                    padding: const EdgeInsetsDirectional.symmetric(
