...
/// Jameia `mach_pro_sailor_mine_settings` (Mine → Settings, v0.0.41) — faithful
/// 1:1 clone of the account settings surface.
///
/// Pure UI / device-preference state, so this screen owns its toggles locally
/// (a [StatefulWidget]) instead of going through a cubit + [JameiaRepository] —
/// there is no server-backed settings data in the dummy repository. Jameia's
/// real screen drives `getMyPageSettingModule` / `setUserInfo`; here the rows
/// flip in-place and Log out / Clear cache are demo no-ops.
///
/// Sections (Jameia order): Language (EN / AR), Notifications switch, Account &
...
class _MineSettingsPageState extends State<MineSettingsPage> {
  ...
  Future<void> _confirmLogout() async {
    // Jameia's log-out confirmation is a CENTERED modal dialog (bundle class
    // `g8998f`: 24dp horizontal margins, 8dp radius, scale-in `a06000` + fade-in
    // `da8c88`, 300ms ease-in-out) — not a bottom sheet. [showJameiaDialog]
    ...
    // Revokes the refresh token server-side and wipes the keychain session
    // (the local wipe happens even offline), then back to login.
...
/// Red full-width log-out button (Jameia's destructive CTA styling).
...
///
/// Mirrors Jameia's bundle card `g8998f` (centered, 24dp horizontal margins, 8dp
