...
/// Edit-profile form: loads the live customer, holds the draft, saves the diff
/// through `PATCH /v1/account/profile`.
///
/// Page-scoped (`registerFactoryParam`): pass the customer the app already
/// knows so the form renders instantly, then [load] refreshes it.
    ...
    result.fold(
      ...
      (customer) => safeEmit(
        state.copyWith(
          ...
          clearGender: customer.gender == null,
          showErrors: false,
        ),
      ),
    );
