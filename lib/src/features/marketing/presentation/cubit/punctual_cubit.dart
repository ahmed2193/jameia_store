...
class PunctualCubit extends Cubit<PunctualState>
  ...
  Future<void> load() async {
    ...
    result.fold(
      (failure) => safeEmit(
        state.copyWith(status: PunctualStatus.error, error: failure.message),
      ),
      (landing) => safeEmit(
        state.copyWith(status: PunctualStatus.loaded, landing: landing),
      ),
    );
  }
  ...
  void toggleFaq(int index) {
    ...
    safeEmit(
      state.copyWith(expandedFaq: state.expandedFaq == index ? -1 : index),
    );
  }
}
