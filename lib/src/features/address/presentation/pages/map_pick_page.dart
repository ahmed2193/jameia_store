...
/// Result of [MapPickPage] — the chosen delivery coordinate + resolved address.
...
/// Jameia "العنوان / Address" map location-picker: a full-screen interactive map
...
class MapPickPage extends StatefulWidget {
  const MapPickPage({super.key, this.initial});
...
class _MapPickPageState extends State<MapPickPage> {
  ...
  void initState() {
    super.initState();
    _center = widget.initial ?? JameiaGeocode.base;
  ...
  Future<void> _recenter() async {
    await _controller?.animateCamera(
      CameraUpdate.newLatLngZoom(JameiaGeocode.base, 16),
    );
  }
  ...
  void _confirm() {
    Navigator.pop(
      ...
      PickedLocation(
        ...
        area: JameiaGeocode.reverse(_center).area,
