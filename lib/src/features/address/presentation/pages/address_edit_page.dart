...
/// Jameia `mach_pro_sailor_c_address_select_page` (C-PAGE v0.0.136) clone — the
...
/// The SELECT candidate list + search are backed by the async [JameiaLbs] layer
/// (REAL Google Places, locale-aware, debounced + stale-guarded). Editing an
/// existing [JameiaAddress] (or re-opening one with a pin) jumps straight to FORM.
...
class _AddressEditViewState extends State<_AddressEditView> {
  ...
  // True while an async JameiaLbs.nearby() fetch is in flight — the SELECT sheet
  ...
  void initState() {
    ...
    // EDIT-LOAD: the cubit seeds lat/lng to JameiaGeocode.base for a NEW address
    // (non-zero), so lat/lng are used PURELY as the initial map centre here.
    _center = hasPin ? LatLng(s.lat, s.lng) : JameiaGeocode.base;
  ...
  Future<void> _onConfirmLocation() async {
    ...
    final r = await JameiaLbs.reverse(_center, localeId: _localeId);
    if (!mounted) return;
      ...
      body: Stack(
        children: [
          ...
          PositionedDirectional(
            ...
            child: AddressTopBar(
              icon: _confirmed ? JameiaIcons.close : JameiaIcons.back,
