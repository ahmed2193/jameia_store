...
/// the top by the repository to mirror Jameia's ordering. Holds framework-free
/// [JameiaAddressEntity]s; the screen reverse-maps to the core [JameiaAddress]
...
/// Serves the saved-address list from the persisted [JameiaRepository] address
...
class AddressListCubit extends Cubit<AddressListState>
  ...
  Future<void> load() async {
    ...
    result.fold(
      (failure) => safeEmit(
        state.copyWith(status: AddressListStatus.error, error: failure.message),
      ),
      (addresses) => safeEmit(
        state.copyWith(
  ...
  ///
  /// Takes the core [JameiaAddress] (the composed persistence payload popped by
  /// the editor) — a deliberate P2.9 write boundary.
  Future<void> upsert(JameiaAddress a) async {
    final exists = state.addresses.any((e) => e.id == a.id);
    final result = exists
        ? await _repository.updateAddress(a)
        : await _repository.addAddress(a);
    await result.fold(
