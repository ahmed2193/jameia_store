...
/// / recipe imagery is drawn from the in-memory [JameiaRepository] catalogue so
...
class ProductDetailsDummyDataSourceImpl
    implements ProductDetailsDummyDataSource {
  ...
  ProductDetail build(Product product) {
    ...
    return ProductDetail(
      ...
      prepMinutes: catalog.settings.prepTime > 0
          ? catalog.settings.prepTime
          : 10,
  ...
  List<String> _foodImages() {
    ...
    final cats =
        catalog.categories
            .where(
              (c) => !_nonFoodCat.any((k) => c.name.toLowerCase().contains(k)),
            )
            .toList()
  ...
  static String _weightFromName(String name) {
    final m = RegExp(
      r'(\d+(?:\.\d+)?)\s?(g|kg|ml|l|pcs|L|ML|G|KG)\b',
    ).firstMatch(name);
    if (m == null) return '';
