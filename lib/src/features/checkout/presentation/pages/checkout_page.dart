...
/// Jameia tip chips: 0 / 5 / 10 / 15 SAR (exact labels from bytecode).
const List<double> _tipOptions = <double>[0, 5, 10, 15];

/// Jameia checkout (`order_confirm_global`) — address bar w/ type icon, item list,
/// drop-off options w/ real bundle assets, cutlery toggle w/ real switch asset,
/// coupon/voucher row, on-time promise, tips w/ Jameia chip style, payment method
...
class _CheckoutView extends StatelessWidget {
  ...
  Future<void> _openCoupons(BuildContext context) async {
    final picked = await context.push(Routes.orderCoupons);
  ...
  void _showSuccess(BuildContext context, JameiaOrderEntity order) {
    showDialog<void>(
      context: context,
      builder: (_) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            const Icon(JameiaIcons.confirm, color: AppColors.success, size: 56),
        ...
        actions: [
          AppButton(
            ...
            onPressed: () {
              // Close the dialog, pop checkout, then open tracking on top of
              // what was under checkout.
              final router = GoRouter.of(context);
              router.pop();
              router.pop();
              router.push(Routes.orderTracking, extra: order.id);
            },
          ),
        ],
      ),
  ...
  Widget build(BuildContext context) {
    return BlocConsumer<CheckoutCubit, CheckoutState>(
      ...
      builder: (context, cs) {
        ...
        return Scaffold(
          // Real Jameia page bg = #F0F1F5 (system-color-neutral-smallBackground).
          backgroundColor: AppColors.smallBackground,
          appBar: _appBar(),
