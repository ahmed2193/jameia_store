...
/// Real Jameia metrics: h:50dp, border-radius:25dp (pill), bg:#FFE41F,
...
class PlaceOrderBar extends StatelessWidget {
  ...
  /// Jameia shows a secondary-confirm dialog (`ic_secondary_confirm_icon`) before
  /// committing the order: review total + payment, then Cancel / Confirm.
  void _placeOrder(BuildContext context) {
    final cart = context.read<CartCubit>();
    final checkout = context.read<CheckoutCubit>();
    final total = draft.totalFor(
      cart.state.subtotal,
      shop.effectiveDeliveryFee,
    );
    showDialog<void>(
      context: context,
      builder: (dialogContext) => AlertDialog(
        ...
        title: Column(
          ...
          children: [
            Image.asset(
              JameiaAssets.secondaryConfirmIcon,
              width: 48,
              height: 48,
