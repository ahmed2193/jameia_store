...
class OrderActions extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    if (order.isActive) {
      ...
      return Row(
        children: [
          ...
          // ── Cancel order (KeeTa-Regular 14dp error red, no bg) ──────────
          GestureDetector(
            onTap: onCancelOrder,
            child: Text(
    ...
    // Cancelled → refund lifecycle jump (KeeTa order_list → refund_detail)
    ...
    // routes into order_review on tap. We render 5 stars (KeetaIcons.star) and
