...
///
/// The live KeeTa flows hit `saveOrUpdateV2` / `DELUSERADDRESS` / `switchRegion`;
/// here every persisted read/write delegates to the in-memory
/// [KeetaRepository], which owns the `shared_preferences`-backed address book and
/// the active-address / active-region [ValueNotifier]s. The region ANCHORS are a
/// pure `KeetaGeocode` derivation (hard-coded serviceable regions), so they are
/// resolved from the util at call time.
///
/// Delegating to the EXACT [KeetaRepository] operations is what keeps add / edit
...
abstract class AddressLocalDataSource {
  ...
  /// Serviceable-region anchors (`KeetaGeocode.openServiceRegions`).
...
class AddressLocalDataSourceImpl implements AddressLocalDataSource {
  ...
  @override
  List<KeetaAddress> addresses() => catalog.addresses;
  ...
  @override
  List<ServiceRegionItem> serviceRegions() => KeetaGeocode.openServiceRegions();
