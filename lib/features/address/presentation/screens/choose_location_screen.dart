...
/// KeeTa `choose_location_page` (C-PAGE v0.0.42) clone — the serviceable
    ...
    return AppBar(
      ...
      leading: PressScale(
        ...
        child: const Padding(
          ...
          child: Icon(KeetaIcons.back, size: 22, color: AppColors.primaryText),
        ),
      ),
...
/// Trailing single-select check — real KeeTa 16x16 `check` (empty) /
...
class _CheckMark extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return PopScale(
      popKey: selected,
      child: Image.asset(
        selected ? KeetaAssets.checkedIcon : KeetaAssets.checkIcon,
        width: 16,
        height: 16,
      ),
    );
...
class _ConfirmBar extends StatelessWidget {
  ...
  Future<void> _onConfirm(BuildContext context) async {
    ...
    if (cubit.isSwitch(picked.region)) {
      final confirmed = await showKeetaBottomSheet<bool>(
        context,
        backgroundColor: AppColors.white,
        shape: const RoundedRectangleBorder(
          borderRadius: BorderRadius.vertical(
            top: Radius.circular(AppSize.r12), // RE §2.2 select-popup radius-top 12dp
          ),
        ),
    ...
    if (context.mounted) Navigator.pop(context, picked.toModel());
  }
}
