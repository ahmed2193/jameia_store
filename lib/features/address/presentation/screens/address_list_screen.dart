...
import '../../../../core/utils/keeta_geocode.dart' show LabelType;
...
// core KeetaAddress) at the picker-pop / edit-deep-link call sites.
...
/// KeeTa `mine_address` (bundle v0.0.38) clone — the saved-address management
...
/// [KeetaAddress] so a caller (checkout / home address bar) can reuse it.
///
...
/// Per §5 there is NO "Default" pill in real KeeTa — default is surfaced by
...
class _AddressListView extends StatelessWidget {
            ...
            builder: (context, state) {
              return switch (state.status) {
                ...
                AddressListStatus.error => ErrorView(
                  // i18n address_error_content / Keeta_C_Refresh_XPZP
  ...
  Widget _loaded(BuildContext context, List<KeetaAddressEntity> addresses) {
              ...
              children: [
                for (var i = 0; i < addresses.length; i++) ...[
                  ...
                  StaggerEntrance(
                    ...
                    child: _AddressRow(
                      address: addresses[i],
                      // TODO(P2.9-boundary): picker pops the CORE KeetaAddress so
                      // checkout / home consume the shared row unchanged.
                      onTap: () =>
                          Navigator.pop(context, addresses[i].toModel()),
                      // TODO(P2.9-boundary): routing builds AddressEditScreen with
                      // a CORE KeetaAddress arg, so reverse-map before pushing.
                      onEdit: () => _openEditor(context, addresses[i].toModel()),
...
class _AddressRow extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    // Resolve the KeeTa tag enum from the entity's raw code (the entity is
        ...
        child: Padding(
          ...
          child: Row(
            ...
            children: [
              ...
              // Edit pencil.
              _RowAction(
                icon: KeetaIcons.edit,
                color: AppColors.secondaryText,
                onPressed: onEdit,
              ),
              // Delete glyph (RE §5: 32dp tap target).
              _RowAction(
                icon: KeetaIcons.delete,
                color: AppColors.secondaryText,
                onPressed: onDelete,
              ),
            ],
...
String _assetFor(LabelType t) => switch (t) {
  LabelType.home => KeetaAssets.labelHome,
  LabelType.work => KeetaAssets.labelOffice,
  LabelType.hangout => KeetaAssets.labelGathering,
  LabelType.faceDelivery ||
  LabelType.assignedPlace ||
  LabelType.other => KeetaAssets.labelOther,
};
...
/// [showKeetaDialog] for the scale-in + fade entrance.
