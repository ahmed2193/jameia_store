...
export '../../../../core/utils/keeta_geocode.dart'
...
class AddressEditState extends Equatable {
  ...
  AddressSchema get schema => KeetaGeocode.kwSchema;
  ...
  String get briefLine => KeetaGeocode.composeBrief(_struct);

  /// Composed detail line.
  String get detailLine => KeetaGeocode.composeDetail(_struct);
  ...
  bool get canSave {
    for (final spec in KeetaGeocode.kwSchema.fieldsFor(structType)) {
...
/// add / update methods (which delegate to the `KeetaRepository` address book).
class AddressEditCubit extends Cubit<AddressEditState>
  ...
  void seed(KeetaAddress? initial) {
  ...
  static AddressEditState _seed(KeetaAddress? a) {
    return AddressEditState(
      ...
      lat: a?.lat ?? KeetaGeocode.base.latitude,
      lng: a?.lng ?? KeetaGeocode.base.longitude,
      area: a?.area ?? '',
  ...
  Map<AddrField, String> fieldErrors() {
    ...
    for (final spec in KeetaGeocode.kwSchema.fieldsFor(s.structType)) {
  ...
  /// Compose the [KeetaAddress], persist it through the add / update use case,
  /// and return the saved row for the screen to pop with (null on failure).
  Future<KeetaAddress?> save() async {
    safeEmit(state.copyWith(status: AddressEditStatus.saving));
    // TODO(P2.9-boundary): the composed KeetaAddress is the persistence payload
  ...
  /// Build the persisted [KeetaAddress] from the current form state.
  KeetaAddress _compose(AddressEditState s) {
