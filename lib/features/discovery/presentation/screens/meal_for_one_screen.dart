...
/// KeeTa `mach_pro_sailor_c_meal_for_one_main` — the "Meal for One" curated
...
///   • the [ShopCard] feed pulled from [KeetaRepository.shops] via slivers,
...
class MealForOneScreen extends StatelessWidget {
  const MealForOneScreen({super.key, this.title = 'Meal for One'});
    ...
    return SliverAppBar(
      ...
      actions: [
        IconButton(
          ...
          icon: const Icon(KeetaIcons.info, size: 20),
          onPressed: onRules,
        ),
        IconButton(
          tooltip: 'discovery.tooltip_share'.tr(),
          icon: const Icon(KeetaIcons.share, size: 20),
          onPressed: onShare,
        ),
...
///
/// Primary: [KeetaAssets.homeHeaderDefaultBg] rendered as a full-bleed asset
...
class _BannerBackground extends StatelessWidget {
  ...
  /// The KeetaAssets path used as the channel banner.  Swap to any other
  /// `KeetaAssets.*` constant (e.g. [KeetaAssets.placeholder16x9]) to change
  /// the illustration.  An empty string triggers the gradient fallback.
  static const String _channelBannerAsset = KeetaAssets.homeHeaderDefaultBg;
...
/// branded [KeetaAssets.homeGuideNotService] illustration (assets.list.txt:
    ...
    return Center(
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Image.asset(
              KeetaAssets.homeGuideNotService,
              width: 120,
              height: 120,
              fit: BoxFit.contain,
          ...
          children: [
            Row(
              children: [
                ...
                const Spacer(),
                IconButton(
                  icon: const Icon(KeetaIcons.close, size: 18),
                  onPressed: () => Navigator.maybePop(context),
                ),
              ],
            ),
      ...
      child: Row(
        ...
        children: [
          const Padding(
            ...
            child: Icon(KeetaIcons.confirm, size: 14, color: AppColors.success),
          ),
          const SizedBox(width: AppSpacing.s8),
          Expanded(
            child: Text(
