...
class ChannelListScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (_) => ChannelListCubit(sl<KeetaRepository>()),
...
class _ChannelListViewState extends State<_ChannelListView> {
  ...
  void _onScroll() {
    final show = _controller.offset > 320;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      floatingActionButton: _ToTopFab(visible: _showToTop, onTap: _scrollToTop),
      body: SafeArea(
        top: false,
        child: ContentClamp(
          child: Column(
            children: [
              ...
              const _FilterBar(),
              const ThinDivider(),
              Expanded(child: _ShopFeed(onOpenShop: _openShop, controller: _controller)),
            ],
          ),
        ),
      ),
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      ...
      child: AnimatedContainer(
        duration: MotionGuard.duration(context, AppMotion.fast),
        padding:
            const EdgeInsetsDirectional.symmetric(horizontal: AppSpacing.s14),
      ...
      builder: (context, state) {
        if (state.shops.isEmpty) {
          return const EmptyStateView(
            message: 'No shops in this channel yet',
...
class _ToTopFab extends StatelessWidget {
  const _ToTopFab({required this.visible, required this.onTap});
  final bool visible;
  final VoidCallback onTap;
