...
/// KeeTa shop/menu screen — Keeta visual style, jm3eia products_screen DATA FLOW.
///
...
class ShopScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // `Product` across features) — no direct `KeetaRepository` reach-in.
    ...
    return Scaffold(
      // real KeeTa shop page background: #F5F6FA (system-color-neutral-largeBackground)
      backgroundColor: AppColors.mediumBackground,
      body: _ShopMenu(
        shop: shop,
        category: cat,
  ...
  Future<void> _scrollToSection(int i) async {
    ...
    if (initial != null && (initial - _scroll.offset).abs() <= viewport * 1.5) {
      ...
      for (int attempt = 0; attempt < 8; attempt++) {
        ...
        final dest =
            (t ?? _estimateOffset(i, _scroll.position.maxScrollExtent))
                .clamp(0.0, _scroll.position.maxScrollExtent);
  ...
  List<Widget> _bodySlivers() {
    ...
    return [
      ...
      for (int i = 0; i < _sections.length; i++)
        SliverToBoxAdapter(
          key: ValueKey('section_${_subs[_activeSubIndex].id}_${_sections[i].id}'),
