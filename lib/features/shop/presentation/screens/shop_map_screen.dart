...
/// KeeTa shop location map (`mach_pro_sailor_c_shop_map`, bundle 51) — 1:1 clone.
///
/// Full-screen real [KeetaMap] with a single shop pin (`KeetaAssets.shopMarker`)
/// + a user/destination pin (`KeetaAssets.deliveryUserMarker`), a floating
...
///
/// GESTURE-SAFE (master doc §0.1 / §7): the [KeetaMap] platform view is NOT
...
/// the KeeTa map page that shows the merchant's category + locality under the pin.
class ShopMapScreen extends StatefulWidget {
  const ShopMapScreen({super.key, this.shopId = 's1'});
...
class _ShopMapScreenState extends State<ShopMapScreen> {
  ...
  // Real KeeTa asset markers (loaded async). Until they decode we fall back to
  // the hue pins below so the map is never empty.
  Set<Marker>? _markers;

  late final Shop _shop;

  /// True when [ShopMapScreen.shopId] matched no catalogue shop — the screen
  ...
  void initState() {
    ...
    // `Shop` DTO — no direct `KeetaRepository` reach-in).
      ...
      body: Stack(
        children: [
          ...
          PositionedDirectional(
            ...
            bottom: _cardHeight,
            child: KeetaMap(target: _kShopLatLng, zoom: 15.0, markers: markers),
          ),
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            children: [
              KeetaImage.circle(url: shop.logo, size: 44),
...
///
/// Schemes are the canonical KeeTa "navigate via" deep links (master doc §7
...
const _kMapApps = <_MapApp>[
  ...
  _MapApp(
    ...
    icon: KeetaIcons.location,
    scheme:
        'androidamap://navi?sourceApplication=KeeTa&lat={lat}&lon={lng}&dev=0',
