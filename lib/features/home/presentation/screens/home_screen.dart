...
// rebuild the core Product for product_details' ProductDetailScreen (DTO consumer).
...
/// KeeTa Home tab — reference section order (`home_page_main` / `osg_home`):
...
class _HomeViewState extends State<_HomeView> {
  ...
  /// so switching clears it (matches KeeTa/jm3eia store-switch behavior).
  ...
  void _openProductShop(BuildContext context, ProductEntity p) {
    // B2: the taxonomy lookup that used to read sl<KeetaRepository>() directly
    // now routes through the cubit → home repository.
    final arg = context.read<HomeCubit>().shopArgForProduct(p.sku);
    Navigator.pushNamed(context, Routes.shop, arguments: arg);
  }
  ...
  void _openProductDetail(BuildContext context, ProductEntity p) {
    Navigator.of(context).push(
      ...
      KeetaSlideUpRoute(page: ProductDetailScreen(product: p.toModel())),
    );
  }
  ...
  Future<void> _openAddress(BuildContext context) async {
    await Navigator.pushNamed(context, Routes.addressList);
          ...
          ContentClamp(
            ...
            child: CustomScrollView(
              ...
              slivers: [
                ...
                SliverToBoxAdapter(child: _ChannelsStrip(channels: s.channels)),
...
class _FilterTabsBarState extends State<_FilterTabsBar> {
  ...
  void _centerActive() {
    ...
    final chipLeftLocal =
        selfBox.globalToLocal(chipBox.localToGlobal(Offset.zero)).dx;
    final chipCentre = _strip.offset + chipLeftLocal + chipBox.size.width / 2;
    final target = (chipCentre - selfBox.size.width / 2)
...
// ── Promotional channels strip (jm3eia feature collections as KeeTa pills) ─────
