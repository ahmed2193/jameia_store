...
import '../../../shop/presentation/screens/shop_screen.dart' show shopRouteArg;
...
/// Full-screen KeeMart product-detail page (PDP). Pushed directly (via
/// [KeetaSlideUpRoute]) so it slides up from the bottom with an X close.
///
...
class _ProductDetailViewState extends State<_ProductDetailView> {
  ...
  void _openProduct(Product p) {
    Navigator.push(
      context,
      KeetaSlideUpRoute(page: ProductDetailScreen(product: p)),
    );
  }
...
class _PdpAppBar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return SliverAppBar(
      ...
      elevation: 0,
      leading: _TopAction(
        icon: KeetaIcons.close,
        onTap: () => Navigator.maybePop(context),
      ),
      actions: [
        _TopAction(icon: KeetaIcons.share, onTap: () {}),
        _TopAction(icon: KeetaIcons.search, onTap: () {}),
      ...
      flexibleSpace: LayoutBuilder(
        builder: (context, constraints) {
          final collapsed = constraints.maxHeight <= kToolbarHeight + topPad + 8;
          return FlexibleSpaceBar(
            centerTitle: false,
            titlePadding:
                const EdgeInsetsDirectional.only(start: 56, end: 16, bottom: 15),
        ...
        return Stack(
          ...
          children: [
            IconButton(
              ...
              icon: Icon(KeetaIcons.cart,
                  key: cartKey, size: 22, color: AppColors.primaryText),
              splashRadius: 22,
            ),
            if (qty > 0)
      ...
      child: InkWell(
        onTap: brand.categoryId.isEmpty
            ...
            : () => Navigator.pushNamed(context, Routes.shop,
                arguments: shopRouteArg(brand.categoryId)),
        child: Row(
          children: [
            KeetaImage(url: brand.logo, width: 40, height: 40, radius: AppRadius.r6),
...
class _ProductGrid extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final cardW = (MediaQuery.sizeOf(context).width -
            AppSpacing.s16 * 2 -
            AppSpacing.s12) /
        2;
                ...
                children: [
                  Expanded(
                    child: _FeedbackPill(
                      ...
                      selected:
                          state.satisfaction == Satisfaction.notSatisfied,
    ...
    return Row(
      ...
      children: [
        ...
        Text(
          label,
          style:
              AppTextStyles.captionLarge.copyWith(color: AppColors.tertiaryText),
        ),
      ],
    );
  }
...
class _SubHeader extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Text(
      label,
      style: AppTextStyles.headingMedium.copyWith(fontWeight: AppTextStyles.bold),
    );
  }
}
