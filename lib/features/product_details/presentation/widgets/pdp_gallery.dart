...
class _PdpGalleryState extends State<PdpGallery> {
  ...
  Future<void> _openViewer(
      BuildContext context, ProductDetailCubit cubit, int index) async {
    final returned = await Navigator.of(context).push<int>(
      KeetaSlideUpRoute<int>(
        page: PdpImageViewer(
                  ...
                  if (i < total) {
                    return GestureDetector(
                      ...
                      child: Padding(
                        ...
                        child: KeetaImage(url: gallery[i], fit: BoxFit.contain),
                      ),
                    );
                  }
                  return GestureDetector(
...
class _IndicatorPill extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final imageLabel =
        'product.image_of'.tr(namedArgs: {'index': '$index', 'total': '$total'});
      ...
      child: Column(
        ...
        children: [
          ...
          Center(
            child: Text(
              '$kcal',
              style: AppTextStyles.digits(40).copyWith(color: AppColors.martGreen),
            ),
          ),
