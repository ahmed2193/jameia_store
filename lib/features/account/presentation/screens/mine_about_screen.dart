...
/// KeeTa `mach_pro_sailor_mine_about` (Mine → About, v0.0.26) — faithful 1:1
/// clone of the About surface.
///
/// Pure static informational page (no server-backed data), so it is a plain
/// [StatelessWidget] — no cubit / [KeetaRepository] needed. KeeTa's real screen
...
class MineAboutScreen extends StatelessWidget {
  const MineAboutScreen({super.key});
    ...
    return Container(
      ...
      child: Padding(
        ...
        child: Image.asset(
          ...
          fit: BoxFit.contain,
          errorBuilder: (_, _, _) =>
              const Icon(KeetaIcons.info, size: 40, color: AppColors.black),
        ),
      ),
    );
  }
...
// ── Real KeeTa social profiles (followed by "Follow us") ─────────────────────
//
// KeetaIcons (the real `wm_c_iconfont`) has NO brand-specific Facebook /
...
const List<_SocialLink> _kSocialLinks = <_SocialLink>[
  _SocialLink(
    label: 'Facebook',
    handle: 'facebook.com/KeeTaHK',
    icon: Icons.facebook,
    brand: AppColors.link,
  ),
  _SocialLink(
    label: 'Instagram',
    handle: 'instagram.com/keeta.official',
