...
/// KeeTa search results (`c_search_shop`) — committed-query pill bar +
...
class SearchShopScreen extends StatelessWidget {
  const SearchShopScreen({super.key, this.query = ''});
...
class _ResultsViewState extends State<_ResultsView> {
  ...
  void _openProduct(ProductEntity p) => Navigator.of(context).push(
        // TODO(P2.9-boundary): product_details is a separate feature whose
        // screen contract is the core `Product` DTO — rebuild it losslessly from
        // our entity (see util/product_boundary.dart) rather than leaking a
        // search entity across the feature seam.
  ...
  List<ShopEntity> _applyLocal(List<ShopEntity> shops, ShopSort sort) {
    ...
    if (_offersOnly) {
      list = list
          .where(
            (s) =>
                s.promo.isNotEmpty || s.products.any((p) => p.hasDiscount),
          )
