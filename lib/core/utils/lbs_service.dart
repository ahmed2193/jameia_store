...
/// list renders. Mirrors KeeTa's `poi_detail` / `nearby_address` response.
class ResolvedAddress {
  ...
  /// District / neighbourhood (sub-locality), KeeTa's "area".
  final String area;
...
/// Async LBS layer over [KeetaGeocode]. Resolves coordinates to REAL,
/// locale-aware places:
...
class KeetaLbs {
  ...
  static Future<ResolvedAddress> reverse(
    ...
    final offline = KeetaGeocode.reverse(p);
    try {
      await geo.setLocaleIdentifier(_lang(localeId));
  ...
  static Future<List<({String title, String subtitle, LatLng pos})>> nearby(
    ...
    final offsets = KeetaGeocode.nearbyCandidates(p);
  ...
  static Future<List<({String title, String subtitle, LatLng pos})>> search(
    ...
    try {
      await geo.setLocaleIdentifier(_lang(localeId));
      final locations = await geo.locationFromAddress(q);
      final out = <({String title, String subtitle, LatLng pos})>[];
    ...
    return KeetaGeocode.autocomplete(q);
  }
