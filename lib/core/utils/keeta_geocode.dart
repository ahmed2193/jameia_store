...
// schema-driven form, and the saved-address model). Kept here so KeetaGeocode is
// the single source of truth for the offline LBS contract.
// ─────────────────────────────────────────────────────────────────────────────

/// KeeTa `structType` — the place ARCHETYPE that drives which form fields show.
...
enum StructType {
  ...
  /// KeeTa numeric struct code (`regionDefaultStructType=2`).
  final int code;

  static StructType fromCode(int code) =>
      StructType.values.firstWhere((s) => s.code == code,
          orElse: () => StructType.apartment);
}

/// KeeTa `labelType` — the saved-address TAG glyph (Home/Work/…); distinct from
...
enum LabelType {
  ...
  static LabelType fromCode(int code) =>
      LabelType.values.firstWhere((l) => l.code == code,
...
/// Per-structType field set + brief/detail line templates. Mirrors KeeTa's
...
/// Flat struct passed to [KeetaGeocode.composeBrief] / [composeDetail]. The
...
class ServiceRegionItem {
  ...
  /// KeeTa numeric city id (as string).
  final String cityId;
...
/// Offline LBS emulation — a deterministic, networkless stand-in for KeeTa's
...
class KeetaGeocode {
  ...
  /// Kuwait-metro service polygon (closed ring, `lng,lat` order in KeeTa's
  ...
  /// codes KeeTa surfaces in its candidate list (e.g. `7WQJ+3M`).
  ...
  static List<({String title, String subtitle, LatLng pos})> autocomplete(
    ...
    for (final a in areas) {
      if (a.name.toLowerCase().contains(q)) {
        ...
        out.add((title: a.name, subtitle: '${plusCode(pos)}, Kuwait', pos: pos));
      }
    }
    return out;
  }
  ...
  static const AddressSchema kwSchema = AddressSchema({
    StructType.apartment: <AddressFieldSpec>[
      ...
      AddressFieldSpec(
          field: AddrField.buildingName, necessary: true, maxLength: 100),
      AddressFieldSpec(
          field: AddrField.aptNumber, necessary: true, maxLength: 50),
      AddressFieldSpec(
          field: AddrField.unitOrFloor, necessary: true, maxLength: 50),
      AddressFieldSpec(field: AddrField.street, necessary: true, maxLength: 100),
      AddressFieldSpec(field: AddrField.block, necessary: true, maxLength: 50),
      AddressFieldSpec(field: AddrField.avenue, necessary: false, maxLength: 50),
      AddressFieldSpec(
    ...
    StructType.office: <AddressFieldSpec>[
      ...
      AddressFieldSpec(
          field: AddrField.buildingName, necessary: true, maxLength: 100),
      AddressFieldSpec(
          field: AddrField.companyName, necessary: true, maxLength: 50),
      AddressFieldSpec(
          field: AddrField.unitOrFloor, necessary: true, maxLength: 50),
      AddressFieldSpec(field: AddrField.street, necessary: true, maxLength: 100),
      AddressFieldSpec(field: AddrField.block, necessary: true, maxLength: 50),
  ...
  static String composeDetail(AddressStruct s) {
    final second =
        s.structType == StructType.office ? s.companyName : s.aptNumber;
