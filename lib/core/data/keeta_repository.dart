...
/// still come from `assets/data/keeta_data.json`.
///
...
class KeetaRepository {
  ...
  List<KeetaOrder> _userOrders = const [];

  // ── Active address + region (mirrors KeeTa's homeSelectedUserAddress +
  //    com.keeta.home.locate.changed / com.keeta.changed.region broadcasts) ────
  ...
  final ValueNotifier<String?> activeAddressIdNotifier =
      ValueNotifier<String?>(null);

  /// Two-letter active region code (`KW` by default). Switching it broadcasts a
  /// region change.
  final ValueNotifier<String> activeRegionNotifier =
      ValueNotifier<String>('KW');

  /// Last map-picked coordinate ("Last selected location" in the inline sheet).
  LatLng? _lastSelectedLatLng;

  // ── Home reference modules (always from keeta_data.json) ───────────────────
  ...
  Future<void> load() async {
    ...
    // Account-side + home reference modules from keeta_data.json.
    ...
    try {
      final assetRaw =
          await rootBundle.loadString('assets/data/jameia/jameia_catalog.json');
      _catalog = await compute(parseJameiaCatalog, assetRaw);
    } catch (_) {
      _catalog = const JameiaCatalog(); // fall back to keeta_data below
    }
  ...
  List<Shop> get bannerRail {
    ...
    return (promoted.isNotEmpty ? promoted : shops).take(8).toList(growable: false);
  }
  ...
  KeetaOrder addOrder(KeetaOrder o) {
  ...
  /// The default address, or a benign empty placeholder ([KeetaAddress.empty])
  /// when the book is empty — never throws (Home/Checkout/Orders read this).
  KeetaAddress get defaultAddress {
    if (_addresses.isEmpty) return KeetaAddress.empty;
  ...
  KeetaAddress get activeAddress {
  ...
  KeetaAddress upsertAddress(KeetaAddress a) {
  ...
  static const String _kAddrPrefsKey = 'keeta.addressbook.v1';
  ...
  Future<void> _persistAddresses() async {
    ...
    } catch (_) {/* best-effort: never block the UI on persistence */}
  }
  ...
  Future<void> _restoreAddresses() async {
    ...
    } catch (_) {/* keep the bundled seed on any decode error */}
  }

  static const String _kOrdersPrefsKey = 'keeta.orders.v1';
  ...
  Future<void> _restoreOrders() async {
    try {
      ...
      _userOrders = (json.decode(saved) as List)
          .map((e) => KeetaOrder.fromJson(e as Map<String, dynamic>))
          .toList();
    } catch (_) {/* keep empty on any decode error */}
  }
  ...
  /// Commit a region switch (`switchRegion` + com.keeta.changed.region). No real
