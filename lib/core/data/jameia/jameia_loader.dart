...
JameiaCatalog parseJameiaCatalog(String raw) {
  ...
  final mediaBase = (meta['mediaBase'] as String?) ?? 'https://media.jm3eia.com';
  ...
  final rawProducts = (root['products'] as Map?)?.cast<String, dynamic>() ?? const {};
  ...
  for (final c in (root['categories'] as List? ?? const [])) {
    ...
    for (final s in (cat['subs'] as List? ?? const [])) {
      ...
      for (final r in (sub['ranks'] as List? ?? const [])) {
        ...
        for (final p in prods) {
          productLocation.putIfAbsent(
              p.sku, () => ProductLocation(catId, subId, rankId));
        }
        ranks.add(JameiaRank(
          id: rankId,
          name: (rk['name'] as String?) ?? '',
          nameAr: (rk['nameAr'] as String?) ?? '',
          image: url(rk['img']),
          count: integer(rk['count']) == 0 ? prods.length : integer(rk['count']),
          products: prods,
        ));
      }
      ...
      for (final p in direct) {
        productLocation.putIfAbsent(
            p.sku, () => ProductLocation(catId, subId, ''));
      }
      subs.add(JameiaSubCategory(
        id: subId,
...
Shop _shopForCategory(JameiaCategory c) {
  ...
  final free = fee == 0.0;
  final time = const ['15-25 min', '20-30 min', '25-35 min', '30-45 min'][seed % 4];
  ...
  final sections = <MenuSection>[
    for (final s in c.subs)
      if (s.allProducts.isNotEmpty)
        MenuSection(id: s.id, title: s.name, image: s.image, products: s.allProducts),
  ];
  ...
  return Shop(
    ...
    minOrder: minOrder,
    tags: [c.name],
    promo: (seed % 3 == 0) ? (free ? 'Free delivery' : '${10 + seed % 30}% off') : '',
    freeDelivery: free,
    sponsored: seed % 9 == 0,
    sections: sections,
  );
