# KeeTa Motion — Reverse-Engineered Reference

Evidence-cited extraction of KeeTa's animation/motion/interaction system from the
...
KeeTa renders UI through **5 runtimes** — native Compose, MTFlexbox→Litho,
...
KeeTa ships **Material3 1.4.0**; the full M3 motion system is bundled.
...
**This is the strongest grounding for KeeTa's actual screen feel.**
...
`v=5.7.1, fr=50, ip=0, op=92 → 1.84s, 500×1084, 11 layers`. Brand wordmark "keeta" +
...
(`keeta_design_loading*.gif` is NOT in this apk — the clone's brand-loader GIF is a separate asset.)
...
- **Order tracking**: `tech_progress_node_{motor,car,delivery,pickup,confirm}_{normal,highlight}.png` (timeline
  ...
  (24-frame heading-rotated marker). → Clone: `_PulseNode`, `KeetaAssets.carMarkerForHeading`.
...
- **Haptics** [FACT, NOT yet in clone]: `performHapticFeedback` (Compose, `-CdsT49E` mangled), `android/os/Vibrator`
  (×15), `HapticFeedbackConstantsCompat`, `isPremiumVibratorEnabled` → KeeTa gives tactile tap feedback,
...
KeeTa, absent in the clone); VAP-style full-screen alpha-video promos; the cart free-delivery 11-frame
progress bar (clone has the PNG frames in `KeetaAssets.cartProgress` but no animated stepping yet).
